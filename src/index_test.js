function toReadable (number) {
  ...
  if (a != 0 && b === 0 && c === 0) {
    return hundr[a-1]} else if (a != 0 && b === 0) {
        return hundr[a-1] + ' ' + digit[c-1]} 
        
        else if (a != 0 && c === 0) {
            return hundr[a-1] + ' ' + ty[b-1]} 
            
            else if (a === 0 && d > 20 && (d % 10) != 0) {
              return ty[b-1] + ' ' + digit[c-1];} 
              
              else if (a === 0 && (d % 10) == 0) {
                return ty[b-1]}   
                                      
                  else if (a===0 && d > 10 || d < 20) {
                    return teen[c-1]} 
                    
                    else if (a===0 && b === 0 && d < 10) {
                      return digit[c-1]} 
                      
                      else if (c != 0 && b != 0 && d > 10 && d < 20) {
                        return hundr[a-1] + ' ' + teen[c-1]} 
                        
                        else {
                          return hundr[a-1] + ' ' + ty[b-1] + ' ' + digit[c-1];}
}

console.log(toReadable (41))
